...
def train(d_model, latent_dim, layers, nqubits, training_samples, discriminator, circuit, n_epochs, samples, lr, hamiltonian1):
    ...
    initial_params = tf.Variable(np.random.uniform(-0.15, 0.15,4*(layers)*nqubits + 2*nqubits-len(fparams)))
    ...
    for i in range(n_epochs):
        ...
        g_loss.append(loss)
        np.savetxt(f"PARAMS_1Dgamma_{nqubits}_{latent_dim}_{layers}_{training_samples}_{samples}_{lr}_fixed_params_{len(fparams)}", [initial_params.numpy()], newline='')
        np.savetxt(f"dloss_1Dgamma_{nqubits}_{latent_dim}_{layers}_{training_samples}_{samples}_{lr}_fixed_params_{len(fparams)}", [d_loss], newline='')
        np.savetxt(f"gloss_1Dgamma_{nqubits}_{latent_dim}_{layers}_{training_samples}_{samples}_{lr}_fixed_params_{len(fparams)}", [g_loss], newline='')
...
if __name__ == "__main__":
    ...
    parser.add_argument("--n_epochs", default=50000, type=int)
