...
def define_cost_gan(params0,params1, d_model0,d_model1, latent_dim, samples, circuit0,circuit1, nqubits, layers, hamiltonian1):
    ...
    real_output0 = d_model0(x_real1, training=True)
    fake_output0 = d_model0(x_fake0, training=True)

    real_output1 = d_model1(x_real0, training=True)
    fake_output1 = d_model1(x_fake1, training=True)
    ...
    loss = discriminator_loss1 + discriminator_loss0 + np.mean(loss_cycle0)+np.mean(loss_cycle1)
    #print(loss0,loss1,loss,loss_cycle0)
...
def train( latent_dim, layers, nqubits, training_samples, discriminator0,discriminator1, circuit0,circuit1, n_epochs, samples, lr, hamiltonian1):
    ...
    for i in range(n_epochs):
        ...
        with tf.GradientTape() as gen_tape0,tf.GradientTape() as gen_tape1, tf.GradientTape() as disc_tape0, tf.GradientTape() as disc_tape1:
            loss = define_cost_gan(initial_params0,initial_params1, discriminator0,discriminator1, latent_dim, samples, circuit0,circuit1, nqubits, layers, hamiltonian1)
        
        grads = gen_tape0.gradient(loss, initial_params0)
        optimizer.apply_gradients([(grads, initial_params0)])
        grads = gen_tape1.gradient(loss, initial_params1)
        optimizer.apply_gradients([(grads, initial_params1)])
       
        gradients_of_discriminator0 = disc_tape0.gradient(
            loss, discriminator0.trainable_weights)

        gradients_of_discriminator1 = disc_tape1.gradient(
            loss, discriminator1.trainable_weights)
