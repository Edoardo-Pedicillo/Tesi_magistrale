...
def define_cycle_cost_gan(params0,params1, latent_dim, samples, circuit0,circuit1, nqubits, layers, hamiltonian1 ):
    ...
    loss_cycle1 = []
    for i in range(5):
        data0 = np.random.logistic(0,0.13,latent_dim*(latent_dim*samples))#generate_training_real_samples(samples*latent_dim,input=0) # logistic
        #data1 = generate_training_real_samples(samples*latent_dim,input=1) # gamma 
        data1 = np.random.gamma(1., size=(latent_dim*samples)*latent_dim)
        data1 = data1/4 - 1
...
def train(d_model, d_model1, latent_dim, layers, nqubits, training_samples, circuit,circuit1, n_epochs, samples, lr, hamiltonian1):
    ...
    for i in range(n_epochs):
        ...
        with tf.GradientTape() as tape:
            loss = define_cost_gan(initial_params, d_model, latent_dim, samples, circuit, nqubits, layers, hamiltonian1) +  define_cycle_cost_gan(initial_params,initial_params1, latent_dim, samples, circuit,circuit1, nqubits, layers, hamiltonian1 )
        ...
        with tf.GradientTape() as tape1:
            loss1 = define_cost_gan(initial_params1, d_model1, latent_dim, samples, circuit1, nqubits, layers, hamiltonian1)+define_cycle_cost_gan(initial_params,initial_params1, latent_dim, samples, circuit,circuit1, nqubits, layers, hamiltonian1 )
        grads1 = tape1.gradient(loss1, initial_params1)
        optimizer.apply_gradients([(grads1, initial_params1)])
        g_loss1.append(loss1)
        np.savetxt(f"PARAMS_cGAN1_{nqubits}_{latent_dim}_{layers}_{training_samples}_{samples}_{lr}", [initial_params.numpy()], newline='')
        np.savetxt(f"dloss_cGAN1_{nqubits}_{latent_dim}_{layers}_{training_samples}_{samples}_{lr}", [d_loss], newline='')
